...
export class SW_WizhMenu extends Phaser.GameObjects.Container
{
    ...
    constructor(scene: SW_BaseScene, x: number, y: number) {
        ...
        this.headerText = this.scene.add.text(this.header.x, this.header.y - 8, "Wizhes".toUpperCase(), { color: SW_CST.STYLE.COLOR.WHITE, strokeThickness: 3, stroke: SW_CST.STYLE.COLOR.BLACK, fontSize: "23px", fontFamily: SW_CST.STYLE.TEXT.FONT_FAMILY }).setOrigin(0.5);
        this.add(this.headerText);

        const makeAWizhText = this.scene.add.text(this.header.x, this.header.y + this.header.height * 0.5 + 20, "You found the magic fountain", { color: SW_CST.STYLE.COLOR.WHITE, strokeThickness: 3, stroke: SW_CST.STYLE.COLOR.BLACK, fontSize: "21px", fontFamily: SW_CST.STYLE.TEXT.FONT_FAMILY }).setOrigin(0.5, 0);
