...
export class SW_InventoryCounterWidget extends Phaser.GameObjects.Container {
  ...
  constructor(scene: SW_BaseScene, x: number, y: number) {
    ...
    const bg = this.scene.add
      .image(0, 0, 'objectCounterBackground')
      .setOrigin(0.5, 0);
    this.add(bg);

    const plusButton = this.scene.add
      .image(25, 14, 'objectCounterPlusButton')
      .setOrigin(0.5, 0);
    ...
    const minusButton = this.scene.add
      .image(
        plusButton.x,
        plusButton.y + plusButton.displayHeight + 5,
        'objectCounterMinusButton'
      )
      .setOrigin(0.5, 0);
    ...
    const moveButton = this.scene.add
      .image(0, bg.y + bg.displayHeight - 16, 'objectCounterMoveButton')
      .setOrigin(0.5, 1);
    ...
    this.quantityText = this.scene.add
      .text(-10, 20, '1', {
        color: 'black',
        fontSize: '24px',
        fontStyle: 'bold',
