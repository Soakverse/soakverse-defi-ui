...
export class SW_TextButton extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    y: number,
    text: string,
    style?: SW_TextButtonStyle
  ) {
    super(scene, x, y);
    scene.add.existing(this);

    this.textureNormal =
      style && style.textureNormal ? style.textureNormal : 'menuButtonNormal';
    this.texturePressed =
      style && style.texturePressed
        ? style.texturePressed
        : 'menuButtonPressed';
    this.textureHovered =
      style && style.textureHovered ? style.textureHovered : 'menuButtonNormal';
    this.textureDisabled =
      style && style.textureDisabled
        ? style.textureDisabled
        : 'menuButtonNormal';

    this.textOffsetNormalY =
      style && style.textOffsetNormalY !== undefined
        ? style.textOffsetNormalY
        : 0;
    this.textOffsetHoveredY =
      style && style.textOffsetHoveredY !== undefined
        ? style.textOffsetHoveredY
        : 0;
    this.textOffsetPressedY =
      style && style.textOffsetPressedY !== undefined
        ? style.textOffsetPressedY
        : 1;
    ...
    this.buttonText = this.scene.add.text(0, 0, text, {
      ...
      fontSize: '19px',
      color: SW_CST.STYLE.COLOR.LIGHT_GREY,
