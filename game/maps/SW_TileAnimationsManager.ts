...
export class SW_TileAnimationsManager extends Phaser.Events.EventEmitter {
  ...
  private declare tileAnimationData: SW_TileAnimationData | undefined;

  private animatedTilesPerLayer: Map<
    string /** Layer id */,
    Phaser.GameObjects.Sprite[] /** Animated tiles */
  >;
  ...
  constructor(mapManager: SW_MapManager) {
    ...
    if (this.scene.cache.json.exists(this.tileAnimationJsonName)) {
      ...
      this.scene.load.once(
        `${Phaser.Loader.Events.FILE_KEY_COMPLETE}json-${this.tileAnimationJsonName}`,
        this.onTileAnimationJsonLoaded,
        this
      );
      this.scene.load.json(
        this.tileAnimationJsonName,
        `/game/assets/maps/${this.worldName}/tileAnimations/tileAnimations.json`
      );
  ...
  private onTileAnimationJsonLoaded(): void {
    this.tileAnimationData = this.scene.cache.json.get(
      this.tileAnimationJsonName
    ) as SW_TileAnimationData;
    this.spriteSheetAnimToLoad = Object.keys(
      this.tileAnimationData.animList
    ).length;
    ...
    for (const tileTexture in this.tileAnimationData.animList) {
      if (this.scene.textures.exists(tileTexture)) {
        ...
        this.scene.load.once(
          `${Phaser.Loader.Events.FILE_KEY_COMPLETE}spritesheet-${tileTexture}`,
          this.onTileTextureLoaded,
          this
        );
        this.scene.load.spritesheet(
          tileTexture,
          `/game/assets/maps/${this.worldName}/tileAnimations/${tileTexture}.png`,
  ...
  private onTileTextureLoaded(tileTexture: string): void {
    if (!this.tileAnimationData) {
      console.warn(
        'SW_TileAnimationsManager::onTileTextureLoaded - tileAnimationData is undefined'
      );
      return;
    }
    ...
    for (const animConfig of tileTextureData.animConfigs) {
      ...
      this.scene.anims.create({
        key: `${tileTexture}${gid}`,
        frames: this.scene.anims.generateFrameNumbers(`${tileTexture}`, {
  ...
  private clear(): void {
    this.animatedTilesPerLayer.forEach(
      (animatedTiles: Phaser.GameObjects.Sprite[]) => {
    ...
    if (this.tileAnimationData) {
      for (const tileTexture in this.tileAnimationData.animList) {
        ...
        for (const animConfig of tileTextureData.animConfigs) {
          this.scene.anims.remove(
            `${tileTexture}${tileTextureData.startGid + animConfig.gid}`
          );
        }
      ...
      console.warn(
        'SW_TileAnimationsManager::clear - tileAnimationData is undefined'
      );
      return;
    }
  ...
  private onLayerSpawned(
    ...
    if (!this.tileAnimationData) {
      console.warn(
        'SW_TileAnimationsManager::onLayerSpawned - tileAnimationData is undefined'
      );
      return;
    }
    ...
    for (const tileTexture in this.tileAnimationData.animList) {
      ...
      for (const animConfig of tileTextureData.animConfigs) {
        const gid = tileTextureData.startGid + animConfig.gid;
        const animatedTiles = layer.createFromTiles(
          gid,
          -1,
          { key: tileTexture },
          this.scene
