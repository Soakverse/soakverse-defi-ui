...
export declare type FocusType = SW_IInteractable &
...
export class SW_InteractionComponent extends Phaser.GameObjects.Zone {
  ...
  constructor(
    ...
    this.focusNameText = this.scene.add.text(0, 0, 'Name focus', {
      fontSize: '22px',
      color: 'white',
      stroke: 'black',
      strokeThickness: 3,
  ...
  public update(): void {
    ...
    if (
      this.currentFocus &&
      !this.scene.physics.overlap(this.currentFocus, this)
    ) {
  ...
  protected setCurrentFocus(newFocus: FocusType | undefined): void {
    ...
    if (this.currentFocus) {
      ...
      this.focusNameText.setPosition(
        this.currentFocus.x,
        this.currentFocus.y + this.currentFocus.getHintOffsetY()
      );
  ...
  public onInteractableOverlapped(interactable: FocusType): void {
    if (!this.currentFocus) {
      ...
      const distPlayerInteractable =
        Math.abs(this.x - interactable.x) + Math.abs(this.y - interactable.y);
      const distPlayerCurrentFocus =
        Math.abs(this.x - this.currentFocus.x) +
        Math.abs(this.y - this.currentFocus.y);
